 ...
 *  Portions from R-2.15.1  Copyright (C) 1995--2012  The R Core Team.
 *
...
static void reset_stack_limit(void *data)
{
    ...
    R_CStackThreshold = (char *) limit[1];
}
...
void R_CheckStack(void)
{
    ...
    if (((ptrdiff_t)R_CStackDir ^ (R_CStackThreshold - &dummy)) > 0) {
        ...
        if (0) { /* Enable for debugging */
            Rprintf(
            "stack limit reached: dir %d, dummy %llu, start %llu, threshold %llu\n",
             R_CStackDir, (unsigned long long) &dummy, 
	...
	uintptr_t stacklimit[2];
        ...
        stacklimit[1] = (uintptr_t) R_CStackThreshold;
...
/* FROM R-2.15.1: Return a constructed source location (e.g. filename#123) 
   from a srcref.  If the srcref is not valid "" will be returned.  */

static SEXP GetSrcLoc(SEXP srcref)
{
    SEXP srcfile, sep, line, result, e;
    if (TYPEOF(srcref) != INTSXP || length(srcref) < 4) {
	return ScalarString(mkChar(""));
    }
    PROTECT(srcref);
    PROTECT(srcfile = R_GetSrcFilename(srcref));
    PROTECT(e = lang2 (install("basename"), srcfile));
    PROTECT(srcfile = eval (e, R_BaseEnv));
    PROTECT(sep = ScalarString(mkChar("#")));
    PROTECT(line = ScalarInteger(INTEGER(srcref)[0]));
    PROTECT(e = lang4 (install("paste0"), srcfile, sep, line));
...
static SEXP do_gettext(SEXP call, SEXP op, SEXP args, SEXP rho)
{
#ifdef ENABLE_NLS
...
static SEXP do_ngettext(SEXP call, SEXP op, SEXP args, SEXP rho)
{
#ifdef ENABLE_NLS
    const char *domain = "";
    char *buf;
...
static SEXP do_bindtextdomain(SEXP call, SEXP op, SEXP args, SEXP rho)
{
#ifdef ENABLE_NLS
    char *res;
...
static SEXP do_traceback(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    int skip;
...
static SEXP do_addCondHands(SEXP call, SEXP op, SEXP args, SEXP rho)
{
...
static SEXP do_resetCondHands(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    checkArity(op, args);
...
static SEXP do_signalCondition(SEXP call, SEXP op, SEXP args, SEXP rho)
{
...
static SEXP do_dfltWarn(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    const char *msg;
    SEXP ecall;
...
static SEXP do_dfltStop(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    const char *msg;
    SEXP ecall;
...
static SEXP do_getRestart(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    int i;
    SEXP list;
    checkArity(op, args);
...
static SEXP do_addRestart(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    checkArity(op, args);
...
static SEXP do_invokeRestart(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    checkArity(op, args);
    CHECK_RESTART(CAR(args));
...
static SEXP do_addTryHandlers(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    checkArity(op, args);
...
static SEXP do_seterrmessage(SEXP call, SEXP op, SEXP args, SEXP env)
{
    SEXP msg;
...
static SEXP do_printDeferredWarnings(SEXP call, SEXP op, SEXP args, SEXP env)
{
    checkArity(op, args);
...
static SEXP do_interruptsSuspended(SEXP call, SEXP op, SEXP args, SEXP env)
{
...
/* FROM R-2.15.1: These functions are to be used in error messages,
   and available for others to use in the API GetCurrentSrcref returns
   the first non-NULL srcref after skipping skip of them.  If it
...
/* FROM R-2.15.1: Return the filename corresponding to a srcref, or ""
   if none is found */
...
/* -------------------------------------------------------------------------- */
/* Some error reporting procedures used elsewhere, put here to avoid putting  */
/* cold code in hot sections.                                                 */
...
attribute_hidden FUNTAB R_FunTab_errors[] =
{
