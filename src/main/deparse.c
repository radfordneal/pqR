...
static void deparse2buff(SEXP s, LocalParseData *d)
{
	...
	if (TYPEOF(op) == SYMSXP) {
            ...
            if (nargs >= 2 &&  strcmp(opname,"if") == 0) {
                ...
                if (!(d->opts & USESOURCE) || !isString(CADDR(s))) {
                    ...
                    for(i = 0 ; i < n ; i++) {
                        ...
                        writeline(d);
                                ...
                                && (isSymbol(CADDR(s)) || isString(CADDR(s))
                ...
                deparse2buff(CAR(s), d);
                print2buff(" ", d);
                print2buff(translateChar(opname), d);
                print2buff(" ", d);
                linebreak(&lbreak, d);
                deparse2buff(CADR(s), d);
                if (lbreak) {
            ...
            else if (0)  /* alternative that was used when not built-in */
