...
yyreduce:
  ...
  case 52:
    ...
    { (yyval) = xxusermacro((yyvsp[0]), xxnewlist(R_NoObject), &(yyloc)); }
    ...
    { xxpopMode((yyvsp[-1])); (yyval) = xxnewlist((yyvsp[0])); 
     						  if(wCalls)
    	    					      warning(_("bad markup (extra space?) at %s:%d:%d"), 
    	    					            xxBasename, (yylsp[0]).first_line, (yylsp[0]).first_column); 
     						  else
  ...
  case 69:
    ...
    { xxpopMode((yyvsp[-1])); (yyval) = xxnewlist(R_NoObject); }
