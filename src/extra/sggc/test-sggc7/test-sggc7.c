...
/* This test program uses compressed pointers, both small and big
   ...
   type 1) and sequence number for object creation.  Indefinite
   numbers of objects are retained, since the 'e' variable keeps a
   linked list of many numeric objects.  Optional garbage collections
   ...
   the maximum number of segments (default 100), and its second giving
...
{ 
  int segs = argc<2 ? 100 : atoi(argv[1]);
