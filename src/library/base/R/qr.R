...
qr.coef <- function(qr, y)
{
	...
	coef[qr$pivot,] <-
            .Internal (qr_coef_cmplx (qr, y))[ix, ]
...
qr.qy <- function(qr, y)
{
    ...
    if(is.complex(qr$qr)) {
        y <- as.matrix(y)
        if(!is.complex(y)) y[] <- as.complex(y)
        return(.Internal (qr_qy_cmplx (qr, y, 0)))
    }
    a <- attr(qr, "useLAPACK")
    if(!is.null(a) && is.logical(a) && a)
        return(.Internal (qr_qy_real (qr, as.matrix(y), 0)))
    n <- as.integer(nrow(qr$qr))
    if(is.na(n)) stop("invalid nrow(qr$qr)")
    k <- as.integer(qr$rank)
    ny <- as.integer(NCOL(y))
...
qr.qty <- function(qr, y)
{
    ...
    if(is.complex(qr$qr)){
        y <- as.matrix(y)
        if(!is.complex(y)) y[] <- as.complex(y)
        return(.Internal (qr_qy_cmplx (qr, y, 1)))
    }
    a <- attr(qr, "useLAPACK")
    if(!is.null(a) && is.logical(a) && a)
        return(.Internal (qr_qy_real (qr, as.matrix(y), 1)))
